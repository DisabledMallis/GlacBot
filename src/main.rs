...
use tokio::task;
// use regex::Regex;
use image::{io::Reader, ImageOutputFormat, Rgba};
...
async fn download_user_skin(user_id: String) -> Result<Vec<u8>, CustomError> {
    ...
    let user_skin = reqwest::get(user_id)
        .await
        .map_err(|e| CustomError::RequestError(e))?;

    let mut bytes = Cursor::new(
        user_skin
            .bytes()
            .await
            .map_err(|_| CustomError::OtherError)?,
    );
                    ...
                    if prefixed_6.is_some() && prefixed_6.unwrap().to_lowercase() == "glacify" {
                        if !msg.attachments.is_empty() {
                            for attachment in &msg.attachments {
                                ...
                                image
                                    .write_to(&mut new_image, ImageOutputFormat::Png)
                                    .unwrap();
                            ...
                            if let Ok(user_data) = data.json::<UserInfoRequest>().await {
                                ...
                                http.create_message(msg.channel_id)
                                    .files(&[(
                                        &format!("{}-modified.png", user_data.data.player.username),
                                        &new_image,
                                    )])
                                    .exec()
                                    .await?;
